Shader "VT_Terrain"
{
	...
	#pragma vertex SplatmapVert2 finalcolor:SplatmapFinalColor finalprepass:SplatmapFinalPrepass finalgbuffer:SplatmapFinalGBuffer noinstancing
	...
	void SplatmapVert2(inout appdata_full v, out Input data)
	{
		...
		data.tc.xy = TRANSFORM_TEX(v.texcoord, _Control);  // Need to manually transform uv here, as we choose not to use 'uv' prefix for this texcoord.
	...
	void surf(Input IN, inout SurfaceOutput o)
	{
		...
		float4 indexData = tex2D(_VT_IndexTex,  (IN.tc.xy));
		float2 wpos = IN.tc.xy * VT_RootSize;
