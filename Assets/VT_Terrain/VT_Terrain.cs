...
{
    //思路：把叶节点，记录到一个队列里，每帧只对这些叶节点做lod是否发生变化的判断
    public class Node
    {
        /// <summary>
        /// 当前帧所有叶节点
        ...
        /// <param name="physicIndexCount"></param>
        /// <param name="onLoadData"></param>
        /// <returns></returns>
        ...
        private void updateState(Vector2 camPos)
        {
            ...
            if (parent != null)
            {
                var parent_lodSize = parent.calculateLodSize(camPos);
                ...
                {
                    //如果父节点想要更大的LOD并且自己和三个兄弟都是叶子节点，则合并父节点
                    parent.merge();
                    return;
                }
            }

            var lodSize = calculateLodSize(camPos);
            ...
            else if (size > lodSize) //当前尺寸 大于 lod需要的尺寸 需要细分出4个子对象
            {
        ...
        {
            var dis = CalculateClosestPoint(camPos,
                new Vector2(x + size / 2, z + size / 2),
                new Vector2(size / 2, size / 2));
            dis = Mathf.Max(1, Mathf.Sqrt(dis));
            var lod = Mathf.Max(0, (int)(Mathf.Log(dis, 2) + 0.5));
            return 1 << lod;
        }

        /// <summary>
        /// 在二维空间中求一个点到一个AABB最近的距离的平方
        /// </summary>
        /// <param name="pos"></param>
        /// <param name="centerPos"></param>
        /// <param name="aabbExt"></param>
        /// <returns></returns>
    ...
    private void Update()
    {
        ...
        UnityEngine.Profiling.Profiler.BeginSample("updateAllLeavesState");
    ...
    {
        UnityEngine.Profiling.Profiler.BeginSample("onLoadNodeData");
