...
public class VT_Terrain : MonoBehaviour
{
    public class Node
    {
        ...
        /// <summary>
        /// 某节点需要加载贴图资源时回调，因为这种加载一般不做在树结构内
        /// </summary>
        ...
        internal void ceateFullChildren()
        {
            ...
            if (children == null)
            {
                ...
                for (var i = 0; i < 4; i++) children[i].ceateFullChildren();
            }
        }
        ...
        public static Node createRoot(int rootSize, int physicIndexCount, Action<Node> onLoadData)
        {
            ...
            for (var i = 0; i < physicIndexCount; i++)
        ...
        private void updateState(Vector3 relativeCamPos)
        {
            ...
            {
                var parent_lodSize = parent.calculateLodSize(relativeCamPos);
                var allBrothersAreLeaf = true;
                for (var i = 0; i < 4; i++)
                    if (parent.children[i].nodeState != NodeState.Leaf)
            ...
            var lodSize = calculateLodSize(relativeCamPos);
        ...
        private float CalculateClosestPoint(Vector3 pos, Bounds bound)
        {
            var centerPos = bound.center;
            var aabbExt = bound.extents;
            // compute coordinates of point in box coordinate system
            var closestPos = pos - centerPos;
    ...
    private void Start()
    {
        ...
        var mipmapCount = (int)Mathf.Log(indexRT.width, 2);
        print(mipmapCount);
        ...
        if (decalsRoot != null)
        {
            ...
            {
                var xMin = (int)item.bounds.min.x;
                var xMax = Mathf.CeilToInt(item.bounds.max.x);
                var zMin = (int)item.bounds.min.z;
                var zMax = Mathf.CeilToInt(item.bounds.max.z);
        ...
        for (var x = 0; x < rootSize; x++)
        for (var z = 0; z < rootSize; z++)
        {
            var cornerHeights = new Vector4(
            ...
            var heightRange = new Vector2(Mathf.Min(cornerHeights.x, cornerHeights.y, cornerHeights.z, cornerHeights.w),
    ...
    private void threadTerrainLodLoop()
    {
        while (true)
        {
            Thread.Sleep(16);
    ...
    private bool isInFrustum(Node item)
    {
        //这部分 常规的 视锥剔除 性能是严重不足的 所以用视锥的外接圆锥 夹角做更保守剔除
        var dpos = item.aabb.center - Camera_main_position + terrainOffset;
        var dis = dpos.magnitude;

        var rAll = Mathf.Acos(Vector3.Dot(dpos.normalized, Camera_main_forward));
        var rNode = Mathf.Atan(item.aabb.extents.magnitude / dis);
        var rFrustun = Camera_main_fov / 2 * Mathf.Max(1, Camera_main_aspect) * Mathf.Deg2Rad;
        var inF = rAll < rNode + rFrustun;
        return inF;
    }
